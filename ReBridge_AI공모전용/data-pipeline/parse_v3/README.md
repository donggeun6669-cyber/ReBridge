...
python3 extract_offerings.py --force  # 전체(876문서, 약 20분)
python3 check_golden.py               # 정답 묶음 대조 — 추출기를 고칠 때마다 **먼저** 돌린다
python3 regression_merge.py           # 병합기 회귀검사(임시 폴더에서 돌아 정본을 건드리지 않는다)
python3 regression_extract.py         # 추출기 회귀검사(가짜 요강 PDF 를 만들어 시험한다)
```

두 회귀검사는 정본을 건드리지 않는다. 환경변수 `PARSE_V3_WORK`(작업대)와 `PARSE_V3_RAW`(원문 폴더)를
임시 폴더로 돌려서 돈다. 코드 안에서 정본 경로를 바꾸지 않기 위한 장치다.
...
| **정본에 쓰는 길은 둘뿐** (2026-09-21) | ① 규칙·조건 계열 → `agent_out/*.jsonl` → `merge_agent.py`(출처 `merge_log`) ② 입시결과 → `parse_results.py`(출처 `document_id`+`processing_log`). `validate.py` 「7-2」가 **출처 없는 행 0** 을 확인한다 |
| **문서별 저장·원자적 교체** (2026-09-21) | 추출 결과는 문서마다 파일 하나(`doc_<id>.jsonl`). 임시 파일에 쓴 뒤 이름을 바꿔 끼운다. 대학 묶음은 실행 끝에 다시 만든다 |
| **행별 검증** (2026-09-21) | 표가 통과해도 **자기 검산이 틀린 행은 `pending`**. 표 전체 통과로 틀린 행을 확정하지 않는다 |
| `regression_extract.py` | 위 두 가지와 '한 쪽의 별개 표 보존', '원문 줄 전체 대조', '정답 대조의 범위 확인' 을 5개 항목으로 검사 |

배경(2026-09-21 외부 교차점검): 이어 돌리면 앞 문서 결과가 지워졌고, 표가 80% 통과하면 틀린 행까지 확정됐고,
한 쪽의 별개 표를 버렸고, 중앙대 정시 결과의 **모집군이 전부 비어 있는데도 정답 검사를 통과**했다. 넷 다 사실이었다.
