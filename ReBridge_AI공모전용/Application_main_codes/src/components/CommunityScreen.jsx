// CommunityScreen — 게시판(에브리타임급).
//   P0: 최신/인기 정렬, 인라인 닉네임 게이트(첫 행동 시 제자리 복귀 — 로그인 화면으로 안 튕김).
//   P1: 검색(현재 보드/전체), 보드 탭 + 인증자 '우리 센터' 보드, 이야기 주제 태그 칩,
//       무한 스크롤, 스크랩(저장) + 내 스크랩 뷰, 신고/차단(액션 시트), 가벼운 알림 뱃지.
...
export default function CommunityScreen({ goTo = () => {}, goBack = () => {} }) {
  ...
  // 공감 토글(목록/스크랩 뷰 공통 갱신).
  const applyLike = useCallback((id, liked) => {
  ...
  const onLike = useCallback(async (e, id) => {
    ...
    if (res.ok) applyLike(id, res.liked);
  }, [user, applyLike]);

  const doBookmark = useCallback(async (id) => {
  ...
  const onBookmark = useCallback(async (id) => {
    if (!user) { setGate({ type: 'bookmark', id }); return; }
    doBookmark(id);
  }, [user, doBookmark]);
  ...
  const onGateDone = useCallback(async () => {
    ...
    if (pending?.type === 'like') {
      ...
      if (res.ok) applyLike(pending.id, res.liked);
    } else if (pending?.type === 'bookmark') {
      doBookmark(pending.id);
    }
  }, [gate, goWrite, applyLike, doBookmark]);
  ...
  const gateModal = <NicknameGate open={!!gate} onClose={() => setGate(null)} onDone={onGateDone} />;
