...
## ⚠️ 커뮤니티는 v1에 들어 있다 — `COMMUNITY_IN_V1 = true` (2026-09-18 동근님)

- 킨텍스 산학연 EXPO(2026-09-30~10-02) 시연용으로 동근님이 v1에 넣었다. **앱 출시 때 뺄지는 다시 정한다** → 빼려면 `src/lib/persona.js`의 `COMMUNITY_IN_V1`만 `false`.
- **시연 모드로만 돈다** — `src/lib/supabaseClient.js`의 `COMMUNITY_DEMO_MODE = true`. 키가 있어도 Supabase에 붙지 않고 기기(localStorage)에 저장한다.
  운영 DB는 익명 로그인이 꺼져 있고, 새 게시판·배지 스키마(`supabase/schema.sql` 맨 아래 2026-09-18 블록)가 **아직 적용 전**이다.
- 게시판(에브리타임 참고): HOT(자동 모음) · 자유 · 고민(익명) · 질문(선생님·멘토 답변) · 정보(검정고시/입시 태그) · 합격 후기 · 꿈드림 후기 (+인증자 우리 센터). 장터·홍보는 두지 않는다.
...
- **메뉴 버튼끼리 같은 화면을 가리키지 않는다.** 단, 빈 화면의 '다음 단계 안내'(예: 관심 대학이 비었을 때 '대학 담으러 가기')는 남긴다.
- 시작 질문 = 인사 → 학년(`profile.grade`) → 사는 지역(`profile.homeRegion`) → 상황. `profile.region`은 '가고 싶은 대학 지역'이라 다른 값이다.
- D-day는 `schedule.js`에서 공고로 확정된 연도에만. 나머지는 '예년 ○월'.
- 3D 아이콘은 `src/assets/icons3d/` (Microsoft Fluent Emoji, MIT — 같은 폴더 LICENSE).

## ⚠️ v1은 대입 트랙만 — `V1_UNIV_ONLY = true` (2026-09-17 다시 켬)

**v1(출시본) 범위는 대입 트랙 + 검정고시 안내 + 지원(꿈드림) + MY + 커뮤니티(2026-09-18 추가)다** (동근님 결정, 2026-09-03 · 2026-09-17 재확인 · 2026-09-18 커뮤니티 추가).
학습 트랙(공부 플래너·학습 로드맵)·직업 트랙·진로 허브는 **v2**다. `main`(=배포본)에서 보이면 안 된다.
